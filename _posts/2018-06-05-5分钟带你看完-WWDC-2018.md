...
16
>开始写自己的第一篇blog，因为自己正在学习如何利用python进行数据分析，所以就写一下如何来使用python进行数据分析吧。
首先我打算写一下如何利用NumPy这个包进行数值计算，虽然NumPy本身不支持数据建模，但是理解Numpy的数组以及基于数组的计算，有助于更高效使用基于数组的共组，比如pandas之类的包。
然后我来简单介绍一下NumPy在数据分析方面的一些应用：

1.在数据处理、清洗、构造子集、过滤、变换以及其他计算中进行快速的向量化计算。

2.常见的数组算法，例如sort、unique以及set操作等。

3.高效的描述性统计和聚合，对数据进行概述。
...
在python中，生成数组的最简单的方式就是使用array函数，array函数接收任意序列型对象（比如列表），生成一个新的包含传递数据的Numpy数组，下面以代码的形式介绍如何导入Numpy，并将一个列表转换成数组：
